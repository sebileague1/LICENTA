...
def send_discord_alert_extended(host, name, severity, cve, is_high, mrbenny_id, solution, is_kev_exploited, soar_action=None, description="N/A", impact="N/A", affected="N/A"):
    try: score = float(severity)
    except: score = 0.0
    
    color = 0x000000 if is_kev_exploited else (0xFF0000 if score >= 7.0 else 0xFF8C00)
    host_display = f"`{host}` | ID: `{mrbenny_id}`"
    ...
    if description and description != "N/A":
        fields.append({"name": "📝 Descriere", "value": f"_{description[:300]}_", "inline": False})
    
    if soar_action:
        fields.append({"name": "🛡️ SOAR Remediation", "value": soar_action, "inline": False})
    else:
        fields.append({"name": "🛠️ Soluție Recomandată", "value": f"*{solution[:400]}*", "inline": False})
    
    payload = {
        "content": "🚨 **VULNERABILITATE NOUĂ DETECTATĂ**" if not is_kev_exploited else "☠️ **CRITICAL KEV MATCH!**",
        "embeds": [{"title": f"🔍 {name[:200]}", "color": color, "fields": fields, "footer": {"text": "SOC OV2 Agent"}}]
    }
...
def run_soar_async(vuln_details):
    try:
        ...
        desc = vuln_details['description']
        
        soar_action = soar_engine.trigger_remediation(host, name) if SOAR_LOADED else None
        send_discord_alert_extended(host, name, str(sev), cve, (sev >= 7.0), mb_id, sol, is_kev, soar_action, desc)
...
def monitor_hids_logs():
    ...
    while True:
        ...
        if not ip or ip in WHITELIST_EXACTE or any(ip.startswith(p) for p in WHITELIST_PREFIXE): continue
        ...
        with QUARANTINE_LOCK:
            ...
            if nou_count >= 3:
                ...
                send_discord_alert_extended("Ubuntu Host", "SSH Brute Force (HIDS)", "10.0", "N/A", True, mb_id, "Carantina SSH", False, f"🛡️ SOAR ACTIV: Blocare automată {ip}", "Atac Brute Force detectat", "Acces neautorizat", "SSH port 22")
